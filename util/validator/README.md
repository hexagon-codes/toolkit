...
- ✅ 正则表达式匹配
- ✅ 自定义规则注册与验证可安全并发执行
- ✅ 未知规则和规则 panic 以可检查错误返回
- ✅ 零外部依赖

字符串长度和密码最小长度按 Unicode 码点计算。结构体标签中的 `oneof=a,b,c` 使用逗号声明枚举值，因此 `oneof` 必须放在该字段验证标签的最后；未知规则会失败关闭，不会被静默忽略。
`FieldError` 只保留字段名、规则、消息和错误分类，不保留被拒绝的原始值，避免密码、令牌等敏感输入滞留在错误对象中。
